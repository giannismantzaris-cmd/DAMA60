...
def tokenize_words(text: str): # The function will return a list of strings (tokens).
    text = _____________ # Lowercase the text so that "Data" and "data" are treated as the same word
    for ch in text:
        if not (ch.isalpha() or ch in "'-"): #Checks if the character is a letter, apostrophe, or hyphen 
            text = text.______________# Replace any character that is NOT a letter (a–z, A–Z), apostrophe ('), or hyphen (-)
...
def trailing_zeros(n: int):
    ...
    if n == 0:
        return 64
    tz = 0
    while ___________: # while least significant bit is 0
        tz = tz + 1
        n = ______________ # shift right (divide by 2), move to next bit
    return tz
...
def fm_estimate(words: List[str], hash_params: List[Tuple[int, int]], m_bits: int = 32): # hash_params: List[Tuple[int, int]]: A list of tuples, each containing two integers (a, b).
    ...
    for w in words: #look at each word from the text one by one
        ...
        word_hash = ____________ # Convert the word into an integer using the custom hash function myHash defined above
        for i, (a, b) in enumerate(hash_params):
            hv = _________________   # Compute hash value in range [0, M)
    ...
    combined = ______________________  # Compute median using python library statistics 
