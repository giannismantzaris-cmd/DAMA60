...
countries = pd.read_csv(_______, sep=",", header=_______, quotechar='"')
...
kmeans = KMeans(_______, init='k-means++', max_iter=200, n_init=20)
clusters = kmeans.fit(_______)

# Plot data and clusters
plt.figure(figsize=(8, 6))
scatter = plt.scatter(countryData[_______], countryData[_______], c=kmeans.labels_.astype(float))
...
hcLinkageMatrix = linkage(_______, _______, metric="euclidean")
...
while (True):
      ...
      clusterLabels = cut_tree(hcLinkageMatrix, height=_______).flatten()
      ...
      plt.axhline(y=_______, color='r', linestyle='--')
